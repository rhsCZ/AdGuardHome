...
- For repeated translated label/value rows, call `intl.getMessage(...)` with
  a static key at the render site. Do not hide locale-key selection behind
  helpers or components that accept translation-key props.
- Rich translation entries must use named tags such as `<span>` or
  `<strong>`. Do not add numeric placeholders such as `<0>`.
...
- In large route harness tests, prefer shared fixture builders and
  table-driven cases over repeating nearly identical state objects. Remove
  smoke tests that only prove the test runner or matcher setup exists.
...
- For route features with multiple blocks, keep page-level layout styles on
  the route component and move selectors owned by a single block into that
  block's own `.module.pcss` file instead of growing one shared page sheet.
