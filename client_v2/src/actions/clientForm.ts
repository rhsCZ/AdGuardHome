...
import { ClientFormState, RootState } from '../initialState';
...
const buildClientConfig = (form: ClientFormState) => {
    return {
        name: form.name,
...
export const saveClient =
    ...
    async (dispatch: AppDispatch, getState: GetState): Promise<boolean> => {
        ...
        const idErrors: (string | undefined)[] = clientForm.ids.map((id: string) => {
            if (!id.trim()) {
